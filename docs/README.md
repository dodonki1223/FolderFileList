...
**:tada:祝30000ダウンロード達成:tada:**

対象のフォルダの中を解析し、フォルダとファイルの一覧を確認できます。
また解析したフォルダとファイルの一覧をテキストで出力することができます。

**[このソフトをダウンロードする](http://ftp.vector.co.jp/69/57/3265/FolderFileList.zip)**  
...
・フォルダのみ表示(K)  
　フォルダのみ表示します。  
　※このボタンをクリックした場合は拡張子指定(E)、名前(LIKE検索)(Ctrl+F)を使用することは出来ません。
...
## その他
実行中に以下のようなメッセージボックスが出た場合は設定を変更する必要があります。  
このエラーは長いパスがある場合に発生する可能性があります。

![エラー２６０文字](Image/ErrorExceeds260characters.png "エラー２６０文字")

ローカルグループポリシーで下記の項目を有効にし、パソコンを再起動して下さい。  

ローカルコンピューターポリシー→コンピューターの構成→管理用テンプレート→システム→ファイルシステム→Win32の長いパスを有効にする

![エラー設定](Image/ErrorSetting.png "エラー設定")
...
### Ver.2.0.0.0
・ドライブを指定してもエラーにならないよう修正  
・出力文字列フォームに絞り込み機能を追加  
・リスト表示フォームの背景色を変更  
・リスト表示フォームで更新日時の並び替えの不具合を修正
